...
<html lang="en"><head>
  ...
  <title>myworker — chode</title>
  <meta name="description" content="myworker — self-healing Cloudflare Worker">
  ...
  <style>
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    .hero-title{font-size:clamp(56px,9vw,120px);font-weight:700;line-height:1.02;letter-spacing:.04em;color:var(--text);margin:0 0 22px}
...
<p class="hero-sub">myworker — scaffolded by chode<br>
...
export default {
  async fetch(request, env) {
    ...
    const validKey = env.API_KEY || 'myworker-dev-key';

    if (p === '/' || p === '/index.html') return html(PAGE);

    if (p === '/api/status') return json({ ok: true, service: 'myworker', ts: Date.now() });
