...
function cmdNew(name, flags) {
  ...
  var htmlDoc = ['<!DOCTYPE html>','<html lang="sv"><head>','  <meta charset="utf-8">','  <meta name="viewport" content="width=device-width,initial-scale=1">',
    '  <title>'+slug+'</title>','  <meta name="description" content="'+slug+' - chode generated worker">',
    '  <link rel="manifest" href="/manifest.json">','  <link rel="robots.txt" href="/robots.txt">','  <link rel="sitemap" href="/sitemap.xml">',
    '  <link rel="icon" href="/favicon.ico">','  <style>',css,'  </style>','</head>',
    '<body style="font-family:system-ui;max-width:640px;margin:0;padding:0 20px;color:#e2e2ea">',
    '<main class="glass-wrap">','  <div id="logo-wrapper"><canvas id="logo"></canvas></div>',
    '  <div id="logo-bg-wrapper"><canvas id="logo-bg"></canvas></div>','  <!-- END Z6 LOGO -->',
    '  <h1>'+slug+'</h1>','  <p style="font-size:18px;color:#aab">Always-on. Keyless. Verifiable.</p>',
    '  <ul style="line-height:2">\n'+featHtml+'\n  </ul>',
    '  <p style="color:#789;margin-top:32px">Powered by chode self-healing harness</p>',
    '  <p style="color:#789;font-size:13px;margin-top:16px"><a style="color:#7B61FF" href="https://github.com/fogennnnn/chode">github.com/fogennnnn/chode</a></p>',
    '</main>','<footer style="margin-top:48px;width:100vw;margin-left:calc(-50vw + 50%);padding:16px 24px 103px 110px;border-top:1px solid rgba(226,226,234,0.08);background:#0a0a10;position:relative;z-index:1;font-size:13px;color:#9b9baa;box-sizing:border-box">',
    '  <b style="color:#e2e2ea">chode</b> &middot; <a href="mailto:inf@oooooooooo.se" style="color:#a892ff">inf@oooooooooo.se</a>','</footer>',
    logoScript,'</body></html>'].join('\n');
...
async function cmdOmniRoute(subcmd) {
  ...
  else if(subcmd==='open'){var r3=await checkOmniRoute();if(!r3.running){warn('Not running. Starting...');await cmdOmniRoute('start');}var u='http://localhost:'+port;info('  Opening '+u+'...\n');var openCmd=os.platform()==='win32'?'start':(os.platform()==='darwin'?'open':'xdg-open');try{spawn(openCmd,[u],{detached:true,stdio:'ignore'});ok('Opened dashboard');}catch(e){fail('Cannot open: '+e.message);info('  Manual: '+u);}}
