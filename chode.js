...
const CIRCUIT_BREAKER_COOLDOWN_MS = 120000; // 2min cooldown
const PARALLEL_PROVIDERS = 3; // simultaneous provider attempts
...
// ─── Circuit Breaker & Usage Tracking ──────────────────────────────────────────
var circuitBreakers = {};
var usageStats = { day: new Date().toISOString().slice(0,10), providers: {} };
var rateLimits = {}; // {pid: {consecutive429, lastReset}}

function loadUsage() {
  try { var d = JSON.parse(fs.readFileSync(path.join(MONITOR_DIR,'usage.json'),'utf8'));
...
function saveUsage() { fs.mkdirSync(MONITOR_DIR,{recursive:true}); fs.writeFileSync(path.join(MONITOR_DIR,'usage.json'),JSON.stringify(usageStats,null,2),'utf8'); }
...
function recordError(pid, error) {
  if (!usageStats.providers[pid]) usageStats.providers[pid] = { requests:0, tokens:0, latencies:[], errors:0 };
  usageStats.providers[pid].errors++; saveUsage();
function recordRateLimit(pid) {
  ...
  if (r.consecutive429 >= 3) info('  ~  Rate limit detected for ' + (PROVIDERS[pid]?.name || pid) + ' -- backing off');
}
...
// ─── Work Queue (persistent multi-step state) ──────────────────────────────────
...
const PROVIDERS = {
  // ── OmniRoute gateway (136+ models, no auth needed) ──
  // Primary free tier: routes to best available model automatically
  // max_tokens omitted — production output, let the model decide
  omnimix:      { name:'OmniRoute Auto',     category:'free_noauth', requiresKey:null, qualityScore:95, endpoints:[{ type:'chat', url:p=>`http://localhost:${p||OMNIROUTE_PORT}/api/v1/chat/completions`, model:'auto/best-coding', headers:()=>({ 'Content-Type':'application/json' }), body:(k,m)=>JSON.stringify({model:'auto/best-coding',messages:m}), parse:d=>d.choices?.[0]?.message?.content }] },
  omnibestfree: { name:'OmniRoute Best Free',category:'free_noauth', requiresKey:null, qualityScore:93, endpoints:[{ type:'chat', url:p=>`http://localhost:${p||OMNIROUTE_PORT}/api/v1/chat/completions`, model:'auto/best-free', headers:()=>({ 'Content-Type':'application/json' }), body:(k,m)=>JSON.stringify({model:'auto/best-free',messages:m}), parse:d=>d.choices?.[0]?.message?.content }] },
  omnicodefree: { name:'OmniRoute Code Free',category:'free_noauth', requiresKey:null, qualityScore:91, endpoints:[{ type:'chat', url:p=>`http://localhost:${p||OMNIROUTE_PORT}/api/v1/chat/completions`, model:'auto/coding:free', headers:()=>({ 'Content-Type':'application/json' }), body:(k,m)=>JSON.stringify({model:'auto/coding:free',messages:m}), parse:d=>d.choices?.[0]?.message?.content }] },
  // OmniRoute tiered models
  omnibest:     { name:'OmniRoute Best',     category:'free_noauth', requiresKey:null, qualityScore:90, endpoints:[{ type:'chat', url:p=>`http://localhost:${p||OMNIROUTE_PORT}/api/v1/chat/completions`, model:'auto/best-coding', headers:()=>({ 'Content-Type':'application/json' }), body:(k,m)=>JSON.stringify({model:'auto/best-coding',messages:m}), parse:d=>d.choices?.[0]?.message?.content }] },
  omnireason:   { name:'OmniRoute Reason',   category:'free_noauth', requiresKey:null, qualityScore:88, endpoints:[{ type:'chat', url:p=>`http://localhost:${p||OMNIROUTE_PORT}/api/v1/chat/completions`, model:'auto/best-reasoning', headers:()=>({ 'Content-Type':'application/json' }), body:(k,m)=>JSON.stringify({model:'auto/best-reasoning',messages:m}), parse:d=>d.choices?.[0]?.message?.content }] },
  omnifast:     { name:'OmniRoute Fast',     category:'free_noauth', requiresKey:null, qualityScore:85, endpoints:[{ type:'chat', url:p=>`http://localhost:${p||OMNIROUTE_PORT}/api/v1/chat/completions`, model:'auto/best-fast', headers:()=>({ 'Content-Type':'application/json' }), body:(k,m)=>JSON.stringify({model:'auto/best-fast',messages:m}), parse:d=>d.choices?.[0]?.message?.content }] },
  omnichat:     { name:'OmniRoute Chat',     category:'free_noauth', requiresKey:null, qualityScore:82, endpoints:[{ type:'chat', url:p=>`http://localhost:${p||OMNIROUTE_PORT}/api/v1/chat/completions`, model:'auto/best-chat', headers:()=>({ 'Content-Type':'application/json' }), body:(k,m)=>JSON.stringify({model:'auto/best-chat',messages:m}), parse:d=>d.choices?.[0]?.message?.content }] },
  omnichaos:    { name:'OmniRoute Chaos',    category:'free_noauth', requiresKey:null, qualityScore:75, endpoints:[{ type:'chat', url:p=>`http://localhost:${p||OMNIROUTE_PORT}/api/v1/chat/completions`, model:'auto/chaos', headers:()=>({ 'Content-Type':'application/json' }), body:(k,m)=>JSON.stringify({model:'auto/chaos',messages:m}), parse:d=>d.choices?.[0]?.message?.content }] },
  // OmniRoute specific models
  omniglm:      { name:'OmniRoute GLM',      category:'free_noauth', requiresKey:null, qualityScore:80, endpoints:[{ type:'chat', url:p=>`http://localhost:${p||OMNIROUTE_PORT}/api/v1/chat/completions`, model:'auto/glm', headers:()=>({ 'Content-Type':'application/json' }), body:(k,m)=>JSON.stringify({model:'auto/glm',messages:m}), parse:d=>d.choices?.[0]?.message?.content }] },
  omnigemini:   { name:'OmniRoute Gemini',   category:'free_noauth', requiresKey:null, qualityScore:84, endpoints:[{ type:'chat', url:p=>`http://localhost:${p||OMNIROUTE_PORT}/api/v1/chat/completions`, model:'auto/gemini', headers:()=>({ 'Content-Type':'application/json' }), body:(k,m)=>JSON.stringify({model:'auto/gemini',messages:m}), parse:d=>d.choices?.[0]?.message?.content }] },
  omnillama:    { name:'OmniRoute Llama',    category:'free_noauth', requiresKey:null, qualityScore:78, endpoints:[{ type:'chat', url:p=>`http://localhost:${p||OMNIROUTE_PORT}/api/v1/chat/completions`, model:'auto/llama', headers:()=>({ 'Content-Type':'application/json' }), body:(k,m)=>JSON.stringify({model:'auto/llama',messages:m}), parse:d=>d.choices?.[0]?.message?.content }] },
  omnimimo:     { name:'OmniRoute MiMo',     category:'free_noauth', requiresKey:null, qualityScore:76, endpoints:[{ type:'chat', url:p=>`http://localhost:${p||OMNIROUTE_PORT}/api/v1/chat/completions`, model:'auto/mimo', headers:()=>({ 'Content-Type':'application/json' }), body:(k,m)=>JSON.stringify({model:'auto/mimo',messages:m}), parse:d=>d.choices?.[0]?.message?.content }] },
  // ── Direct no-auth providers (fallback if OmniRoute unavailable) ──
  deepseek:     { name:'DeepSeek (Free)',  category:'free_noauth', requiresKey:null, qualityScore:82, endpoints:[{ type:'chat', url:'https://api.deepseek.com/beta/chat/completions', model:'deepseek-chat', headers:()=>({ 'Content-Type':'application/json' }), body:(k,m)=>JSON.stringify({model:'deepseek-chat',messages:m,stream:false}), parse:d=>d.choices?.[0]?.message?.content }] },
  ...
  ai_horde:     { name:'AI Horde',         category:'free_noauth', requiresKey:null, qualityScore:65, endpoints:[{ type:'chat', url:'https://corsproxy.io/?https://ai.api.aihorde.net/api/v2/chat/completions', model:'none', headers:()=>({ 'Content-Type':'application/json' }), body:()=>JSON.stringify({model:'none',messages:[{role:'user',content:'say ok'}]}), parse:d=>d.choices?.[0]?.message?.content }] },
  // ── OmniRoute individual providers ──
  aug:          { name:'Auggie (OmniRoute)',category:'free_noauth', requiresKey:null, qualityScore:92, endpoints:[{ type:'chat', url:p=>`http://localhost:${p||OMNIROUTE_PORT}/api/v1/chat/completions`, model:'aug/sonnet4.6', headers:()=>({ 'Content-Type':'application/json' }), body:(k,m)=>JSON.stringify({model:'aug/sonnet4.6',messages:m}), parse:d=>d.choices?.[0]?.message?.content }] },
  tllm:         { name:'The Old LLM',      category:'free_noauth', requiresKey:null, qualityScore:85, endpoints:[{ type:'chat', url:'https://theoldllm.com/api/chat', model:'tllm/GPT_5_4', headers:()=>({ 'Content-Type':'application/json' }), body:(k,m)=>JSON.stringify({model:'tllm/GPT_5_4',messages:m}), parse:d=>d.choices?.[0]?.message?.content }] },
  oc:           { name:'OpenCode Free',    category:'free_noauth', requiresKey:null, qualityScore:80, endpoints:[{ type:'chat', url:'https://opencode.ai/api/chat', model:'oc/deepseek-v4-flash-free', headers:()=>({ 'Content-Type':'application/json' }), body:(k,m)=>JSON.stringify({model:'oc/deepseek-v4-flash-free',messages:m}), parse:d=>d.choices?.[0]?.message?.content }] },
  ddgw:         { name:'DuckDuckGo AI',    category:'free_noauth', requiresKey:null, qualityScore:72, endpoints:[{ type:'chat', url:'https://duckduckgo.com/?duck_ai=1', model:'ddgw/gpt-5.4-mini', headers:()=>({ 'Content-Type':'text/html' }), body:()=>'<query>', parse:d=>d?.match(/<div class="aI">([\s\S]*?)<\/div>/)?.[1]?.replace(/<[^>]*>/g,'')?.trim() || 'DuckDuckGo AI' }] },
  agentrouter:  { name:'AgentRouter',      category:'free_noauth', requiresKey:null, qualityScore:88, endpoints:[{ type:'chat', url:p=>`http://localhost:${p||OMNIROUTE_PORT}/api/v1/chat/completions`, model:'agentrouter/claude-opus-4-6', headers:()=>({ 'Content-Type':'application/json' }), body:(k,m)=>JSON.stringify({model:'agentrouter/claude-opus-4-6',messages:m}), parse:d=>d.choices?.[0]?.message?.content }] },
  felo:         { name:'Felo AI',          category:'free_noauth', requiresKey:null, qualityScore:70, endpoints:[{ type:'chat', url:'https://felo.ai/api/chat', model:'felo/felo-chat', headers:()=>({ 'Content-Type':'application/json' }), body:(k,m)=>JSON.stringify({model:'felo/felo-chat',messages:m}), parse:d=>d.choices?.[0]?.message?.content }] },
  // ── API key providers ──
  perplexity:   { name:'Perplexity',       category:'free_tier',   requiresKey:'PERPLEXITY_API_KEY', qualityScore:90, endpoints:[{ type:'chat', url:'https://api.perplexity.ai/chat/completions', model:'sonar', headers:k=>({ 'Content-Type':'application/json','Authorization':'Bearer '+k }), body:(k,m)=>JSON.stringify({model:'sonar',messages:m}), parse:d=>d.choices?.[0]?.message?.content }] },
  anthropic:    { name:'Claude',           category:'paid',      requiresKey:'ANTHROPIC_API_KEY',  qualityScore:100, endpoints:[{ type:'chat', url:'https://api.anthropic.com/v1/messages', model:'claude-sonnet-4-20250514', headers:k=>({ 'Content-Type':'application/json','x-api-key':k,'anthropic-version':'2023-06-01' }), body:(k,m)=>JSON.stringify({model:'claude-sonnet-4-20250514',messages:m}), parse:d=>d.content?.[0]?.text }] },
  openai:       { name:'GPT-4o-mini',      category:'paid',      requiresKey:'OPENAI_API_KEY',       qualityScore:95, endpoints:[{ type:'chat', url:'https://api.openai.com/v1/chat/completions', model:'gpt-4o-mini', headers:k=>({ 'Content-Type':'application/json','Authorization':'Bearer '+k }), body:(k,m)=>JSON.stringify({model:'gpt-4o-mini',messages:m}), parse:d=>d.choices?.[0]?.message?.content }] },
  ...
  ollama:       { name:'Ollama',           category:'free_local',requiresKey:null,                 qualityScore:60, endpoints:[{ type:'chat', url:()=>(process.env.OLLAMA_URL||'http://localhost:11434')+'/api/generate', model:process.env.OLLAMA_MODEL||'llama3.2', headers:()=>({ 'Content-Type':'application/json' }), body:(k,m)=>JSON.stringify({model:process.env.OLLAMA_MODEL||'llama3.2',prompt:m[m.length-1].content,stream:false}), parse:d=>d.response }] },
  groq:         { name:'Groq',             category:'free_tier', requiresKey:'GROQ_API_KEY',      qualityScore:88, endpoints:[{ type:'chat', url:'https://api.groq.com/openai/v1/chat/completions', model:'llama-3.3-70b-versatile', headers:k=>({ 'Content-Type':'application/json','Authorization':'Bearer '+k }), body:(k,m)=>JSON.stringify({model:'llama-3.3-70b-versatile',messages:m}), parse:d=>d.choices?.[0]?.message?.content }] },
  cerebras:     { name:'Cerebras',         category:'free_tier', requiresKey:'CEREBRAS_API_KEY',  qualityScore:82, endpoints:[{ type:'chat', url:'https://api.cerebras.ai/v1/chat/completions', model:'llama-3.1-70b', headers:k=>({ 'Content-Type':'application/json','Authorization':'Bearer '+k }), body:(k,m)=>JSON.stringify({model:'llama-3.1-70b',messages:m}), parse:d=>d.choices?.[0]?.message?.content }] },
  ...
  gemini:       { name:'Gemini CLI',       category:'free_tier', requiresKey:'GEMINI_API_KEY',    qualityScore:88, endpoints:[{ type:'chat', url:k=>`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${k}`, model:'gemini-2.5-flash', headers:()=>({ 'Content-Type':'application/json' }), body:(k,m)=>{var c=m.map(x=>({role:x.role==='assistant'?'model':'user',parts:[{text:x.content}]})); return JSON.stringify({contents:c});}, parse:d=>d.candidates?.[0]?.content?.parts?.[0]?.text }] },
  mistral:      { name:'Mistral',          category:'free_tier', requiresKey:'MISTRAL_API_KEY',   qualityScore:87, endpoints:[{ type:'chat', url:'https://api.mistral.ai/v1/chat/completions', model:'mistral-small', headers:k=>({ 'Content-Type':'application/json','Authorization':'Bearer '+k }), body:(k,m)=>JSON.stringify({model:'mistral-small',messages:m}), parse:d=>d.choices?.[0]?.message?.content }] },
};
...
async function callWithBestProvider(prompt, sessionId, forceProvider) {
  ...
  var viable = candidates.filter(function(pid) {
    ...
    if (p.requiresKey) return !!(process.env[p.requiresKey] || (loadConfig().providers?.[pid]?.key));
  ...
  var batchSize = Math.min(PARALLEL_PROVIDERS || 3, viable.length);
  ...
  if (!result) { fail('\\n  All providers exhausted.'); info('\\n  Set an API key or run chode omniroute start for free tiers.\\n'); return null; }
...
async function trySingleProvider(pid, prompt, session) {
  ...
  return await retry(async function() {
    if (endpoint.type === 'simple') {
      ...
      var body = await resp.text();
      if (resp.ok && body && body.length > 2) {
        return { pid: pid, result: body, latency: Date.now()-t0, tokens: Math.ceil(body.length/4) };
      }
...
function cmdStatus() {
  ...
  for (var pid in PROVIDERS) {
    ...
    var icon = '○'; var status = '';
    if (p.requiresKey) {
      if (hasKey && hasKey.valid !== false) { icon = '✓'; status = 'ready'; }
      else if (hasKey && hasKey.valid === false) { icon = '✗'; status = 'invalid key'; }
      else { status = 'no key'; }
    } else { icon = '✓'; status = 'local/free'; }
    ...
    info('   '+p.name.padEnd(20)+status.padEnd(30)+uStr+'\n','dim');
  }
  
  var cp = loadCheckpoint();
  if (cp) info('\n  Last Checkpoint: step='+(cp.stepIndex||'n/a')+' provider='+(PROVIDERS[cp.lastProvider]?.name||cp.lastProvider)+'\n','yellow');
  ...
  if (queues.length > 0) {
    ...
    for (var k = 0; k < queues.length; k++) {
      var q = queues[k];
      var done = q.steps ? q.steps.filter(function(s){return s.done;}).length : 0;
  ...
  info('  Commands: chode ai "prompt" | chode project "<spec>" | chode status | chode scan\n');
}
